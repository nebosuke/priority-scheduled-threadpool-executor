...
1. **⏰ スケジュール時刻が最優先**: 実行予定時刻が早いものから実行
2. **🎯 同じ時刻なら優先度順**: URGENT → HIGH → NORMAL → LOW の順
3. **📝 同じ優先度なら提出順**: 先に提出されたタスクから実行

### ⚠️ 現在の制限事項

- **キュー割り込み不可**: 既にキューに入ったタスクを高優先度タスクが追い越すことはできません
- **スケジュール時刻優先**: 基本的にはスケジュール時刻順での実行となります
- **同時刻優先度制御**: 同じ実行時刻にスケジュールされたタスク間でのみ優先度が効きます

💡 **真の優先度割り込み機能**（実行待ちキューの動的再編成）を実現するには、より高度なアーキテクチャが必要です。
